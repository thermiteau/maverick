...
3. **Persist the DAG to GitHub** — this is durable for any instance to
   resume from:
   ```bash
   # Write the DAG to a tmp file, then post via the bot for consistent audit trail.
   uv run maverick bot gh -- issue comment  --body-file /tmp/dag-marker.md
   ```
   Alternatively, the CLI helper composes the marker for you — preferred:
   construct the payload JSON and post via the bot's `gh` with a fenced
   `maverick-dag` block.
...
- **The bot is mandatory.** Abort on entry if not configured.
