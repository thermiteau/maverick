...
All projects must maintain a **minimum 60% coverage** for integration tests. Integration test coverage measures the percentage of cross-boundary code paths exercised by the test suite.

### What "Coverage" Means for Integration Tests

Integration test coverage is measured differently from unit test coverage:

| Metric | What it measures | How to assess |
| --- | --- | --- |
| API endpoint coverage | Percentage of endpoints with at least one integration test | Count tested vs total endpoints |
| Database operation coverage | Percentage of query/mutation patterns exercised | Review repository/DAO layer coverage |
| Service boundary coverage | Percentage of external service integrations tested | Count tested vs total integration points |
| Line coverage during integration runs | Lines executed when integration tests run | Standard coverage tooling with integration test suite |
...
- **CI gate** — configure coverage to fail the build if integration test coverage drops below 60%
- **Separate from unit coverage** — measure integration test coverage independently; do not combine with unit test coverage numbers
- **Coverage reporting** — generate and publish integration test coverage reports alongside unit test coverage in CI

### Practical Guidance

- Start by ensuring all critical paths have integration tests (authentication flows, payment processing, data persistence)
- Expand coverage outward from critical paths to secondary features
- If 60% is not achievable immediately (e.g., legacy codebase), set a ratchet — coverage must not decrease, and each PR should improve it
...
<!-- maverick-plugin-version: 3.2.2-dev -->
