...
A's branch and opens a PR whose base is `feat/A-…` rather than `main`. This
is a **stacked PR**. It lets B progress without waiting for A to merge, and
is essential for throughput inside an epic.

Stacking is safe when it is paired with a **retarget guard** that moves B's
PR base back to `main` as soon as A lands. Without the retarget guard, a
specific merge-order sequence can orphan B's commits — see the failure
mode at the bottom of this document.
...
digraph stack_decide {
    ...
    "Default — branch from main/develop" [shape=box];

    "Depends on sibling story?" -> "Sibling PR open but unmerged?" [label="yes"];
    "Depends on sibling story?" -> "Default — branch from main/develop" [label="no"];
    "Sibling PR open but unmerged?" -> "Stack — branch from sibling" [label="yes"];
    "Sibling PR open but unmerged?" -> "Default — branch from main/develop" [label="no — sibling already merged"];
}
```
...
digraph retarget {
    ...
    "git merge-base check — base fully in default branch?" [shape=diamond];
    "Retarget PR base to default branch" [shape=box];
    "Rebase onto default branch tip" [shape=box];
    "Push as normal" [shape=box];

    "Ready to push" -> "Stacked branch?";
    "Stacked branch?" -> "git merge-base check — base fully in default branch?" [label="yes"];
    "Stacked branch?" -> "Push as normal" [label="no"];
    "git merge-base check — base fully in default branch?" -> "Retarget PR base to default branch" [label="yes — base merged"];
    "git merge-base check — base fully in default branch?" -> "Push as normal" [label="no — base still unmerged"];
    "Retarget PR base to default branch" -> "Rebase onto default branch tip";
    "Rebase onto default branch tip" -> "Push as normal";
}
```
...
```bash
# Is every commit from the sibling base now reachable from default branch?
git fetch origin main feat/140-bootstrap-provider
if git merge-base --is-ancestor origin/feat/140-bootstrap-provider origin/main; then
    # The sibling has fully merged. Retarget and rebase.
    gh pr edit <PR> --base main
    git rebase --onto origin/main origin/feat/140-bootstrap-provider
fi
```

`git merge-base --is-ancestor A B` exits 0 if every commit of A is reachable
from B. After a squash-merge of A into `main`, the squash commit carries
the content but not the commit SHAs, so `--is-ancestor` will be false. Use
instead the `gh pr view --json state` check for the sibling PR:
...
The PR-state check is the authoritative signal because it handles both
merge-commit and squash-merge cases.
...
This strips out commits that are already in `main` (from the sibling's
merge) and replays your commits on top of the current `main`.
...
2. The human merges A → `main` (squash). A's branch still exists on the
   remote but its commits are no longer the base of anything.
3. 25 seconds later the human merges B → A (not B → `main`). GitHub says
   "B merged". The commits of B are fast-forwarded into A's now-orphaned
   branch — **not into `main`**.
4. `main` has A's content but not B's. Discovered much later when B's code
   is "missing in production."

The retarget guard prevents step 3 from being possible: B's base is moved
to `main` the moment A merges, and B's next push either rebases cleanly or
flags a conflict. Either way, B cannot silently merge into an orphan.
