...
Multi-story (`do-epic`) workflows have additional state on GitHub that
...
### The five things to check on re-entry
...
### Block-propagation resume — critical path
...
1. Read the marker payload.
2. For each descendant not already in `labelled`:
   - Apply `blocked-by:#<ejected>` if not present (idempotent).
   - Post the block comment if not present.
   - Update `maverick-state` for that story.
   - Append to `labelled` in the marker.
3. When every descendant is in `labelled`, **delete** the marker.

Only after `maverick-bprop` is cleared should you proceed with other
