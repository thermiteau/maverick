...
**Remote code-review gate.** Verify the project ships a CI workflow that
runs the agent code reviewer on every PR. Per `mav-bp-remote-code-review`,
look for any file under `.github/workflows/` that contains a line equal to
`# maverick:code-review` AND has an `on: pull_request:` trigger:

```bash
grep -lE '^\s*# maverick:code-review\s*$' .github/workflows/*.y*ml 2>/dev/null
```

If no file matches, **stop and offer the user two paths** before claiming
the issue or branching:
...
1. **Scaffold the reference workflow.** Copy
   ...
   with ``. The workflow gates itself on its own PR — a
   useful smoke test. After the workflow merges, record the milestone:
   `uv run maverick integration set code_review_workflow true`.
2. **Abort.** If the user declines to add the workflow, stop here. Do
   not claim the issue, do not branch. Report that the project is not
   compliant with the binary review contract and ask them to run
   `/maverick:do-maverick-alignment` for context.

There is no silent skip path — the auto-merge in Phase 9 trusts the
remote review gate, and proceeding without it would let unreviewed
code land.
