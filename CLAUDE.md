...
- **Git workflow**: Trunk-based with a stable tracking branch.
  ...
  - `stable` is the branch end users install from (it is GitHub's default branch, so a bare `git clone` resolves to it). It is automatically fast-forwarded to the latest release tag by `release-finalize.yml` and never moves during `-dev` cycles. No human ever pushes to `stable` — it is CI-managed. This rule has no carve-outs: never PR into `stable`, never merge `main` into `stable`, never push a hotfix to `stable`. Any non-tag commit on `stable` immediately breaks `/plugin update` for end users (they receive whatever `-dev` version is on the branch). If you need a workflow or doc fix on `stable`, it lands automatically via the next release tag. The `verify-stable.yml` workflow opens a tracking issue when `stable` lands at a non-tag commit.
...
- **Multi-instance workflows**: `do-epic` and the refactored `do-issue-solo` coordinate via GitHub labels and `maverick-*` marker comments (see `docs/conventions/github-markers.md`). They require git worktrees under `.maverick/worktrees/` and a configured Maverick GitHub App (`~/.maverick/config.json` → `gh_app` section; legacy `bot` section is still read with a deprecation warning). Verify with `maverick gh-app status`. Single-machine ad-hoc use still works without the GitHub App — only the auto-merge step needs it.
