...
1. **Spec compliance** — does the PR implement what was asked?
2. **Code quality** — is the implementation correct, secure, maintainable?

**Output:** one of exactly two verdicts.

- `PASS` — the PR may be auto-merged. The caller will run
  `gh pr review --approve` as `maverick-bot` and then `gh pr merge --auto`.
...
- **Correctness** — logic actually works? Off-by-one, race conditions,
  null handling, unvalidated input?
- **Error handling** — errors caught at the right boundaries? Logged with
  ...
  standards?
- **Security** — injection, XSS, auth bypass, secrets exposure, other
  OWASP Top 10? Inputs validated at system boundaries?
...
Anything you would block in a human review is a Stage 2 FAIL.
...
- Security defect (Stage 2) — any of the OWASP Top 10
...
- Dangerous commit contents — secrets, tokens, large binaries (Stage 2)
